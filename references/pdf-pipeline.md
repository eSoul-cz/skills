...
The Docker-built static Go installer records a semantic tool version and SHA-256 checksums for managed files. The host-side entrypoint needs only a POSIX shell and Docker. Bundled assets are always mounted read-only; `--check` also mounts the selected project read-only, while install and explicit upgrade mount only that project root read-write. `--check` reports drift and compares versions without calling a newer installed version an available update. `--upgrade` requires explicit invocation, aborts when managed files were locally modified, and refuses to downgrade a project whose managed version is newer than the bundle. Preserve project configuration, sources, specifications, decisions, and approved assets.
