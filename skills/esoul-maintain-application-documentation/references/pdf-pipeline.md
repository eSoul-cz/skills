...
Run `docs/documentation doctor` before changing profiles or diagnosing a hosted renderer. It reports the installed version and profile, managed-file drift, configuration-schema compatibility, configured digest, Docker and registry readiness, legacy theme settings, and trusted allowlist status. A release-catalog match never authorizes execution by itself: remote validation and rendering require an exact match from the environment or untracked local Git configuration.
