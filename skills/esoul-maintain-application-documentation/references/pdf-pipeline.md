...
The Docker-backed `validate`, `build`, `render`, and `redact` commands require only a POSIX shell and Docker on the host, plus optional Git for ignore-rule and revision checks. The host-level `doctor` and `version` commands do not require Docker to start; `doctor` reports missing or unavailable Docker as part of its diagnostics. The shell wrapper reads only the small set of scalar TOML values required to select/build the renderer image and mount its dedicated writable directories. The Go binary owns configuration, Markdown, publication-policy, build, render, and redaction behavior inside the image. `validate` runs with the entire checkout read-only. `build`, `render`, and `redact` additionally mount only the configured PDF and work directories read-write.
...
The documentation tool distribution is separate from this skill. Treat `docs/documentation` as the stable project interface and project-owned `docs/documentation.toml` as the configuration boundary. Do not depend on the skill installation path for renderer binaries, Lua filters, Dockerfiles, release data, or upgrade scripts.

Run `docs/documentation doctor` before changing profiles or diagnosing a hosted renderer. It reports the installed version and profile, managed-file drift, configuration-schema validity, configured digest, Docker and registry readiness, legacy theme settings, and trusted allowlist status. Git tags and their GitHub Release manifests are authoritative for installation and upgrades; no release catalog is installed or stored in the tooling source. Remote validation and rendering require the configured digest to match the environment or untracked local Git allowlist.

Tool installation and managed upgrades are explicit operations performed through the separately distributed tooling. Preserve project configuration, sources, specifications, decisions, and approved assets. Never replace locally modified managed files without a reviewed plan and explicit approval.
